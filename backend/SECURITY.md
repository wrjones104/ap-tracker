...
- **Token Lifetimes**: JWTs have extended expirations based on the account type (e.g., 90 days for Discord users, 2 years for guest accounts). Revocation relies on the JWT blocklist for immediate access termination.
...
- **Server-Side Request Forgery (SSRF) Protection**: The application uses `SSRFProtectedTCPConnector` and `SSRFProtectedResolver` to prevent requests to private, loopback, link-local, multicast, reserved, and cloud metadata IPs. This prevents attackers from forcing the server to make requests to internal resources. Local testing is permitted when the environment is explicitly set to development.
